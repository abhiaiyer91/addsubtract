# Task: Visual Workflow Editor

## Objective
Create a visual editor for CI workflows that allows users to build and edit workflow YAML files through a drag-and-drop interface.

## Context

### Current State
- Workflows are edited manually as YAML files
- No visual representation during editing
- Syntax errors discovered only on run
- No auto-completion or validation feedback
...
- Auto-complete for action inputs
- Live preview of execution flow

## Technical Requirements

### 1. Workflow Editor Page (`apps/web/src/routes/repo/workflow-editor.tsx`)

```tsx
import { useState, useCallback } from 'react';
import { useParams, useNavigate, useSearchParams } from 'react-router-dom';
import { Save, Play, Eye, Code, Plus, Trash2, GripVertical } from 'lucide-react';
...
export function WorkflowEditor() {
  ...
  const { data: existingContent } = trpc.repos.getFileContent.useQuery(
    { owner: owner!, repo: repo!, path: editPath!, ref: 'main' },
    { enabled: !!editPath }
  );
  ...
  // Sync YAML when workflow changes
  useEffect(() => {
    if (activeTab === 'visual') {
  ...
  // Parse YAML back to workflow data
        ...
        <div className="space-y-4">
          {Object.entries(workflow.jobs).map(([jobId, job]) => (
            <JobEditor
              key={jobId}
              job={job}
              allJobs={Object.keys(workflow.jobs)}
...
### 3. Job Editor Component

```tsx
function JobEditor({ job, allJobs, onChange, onDelete }: {
  job: WorkflowJob;
  allJobs: string[];
  onChange: (updates: Partial<WorkflowJob>) => void;
  onDelete: () => void;
}) {
  const addStep = (template?: Partial<WorkflowStep>) => {
    ...
    <Card>
      ...
      <CardContent>
        ...
        <Button
          ...
          onClick={() => addStep({ run: 'echo "Hello"' })}
        >
          <Plus className="h-4 w-4 mr-1" />
          Add Step
        </Button>
...
const STEP_TEMPLATES = [
  ...
  },
  {
    ...
    steps: [
      ...
      { name: 'Run custom script', run: 'echo "Add your command"' },
    ],
  },
];

function StepLibrary() {
  const handleDragStart = (step: Partial<WorkflowStep>) => (e: React.DragEvent) => {
          ...
          <div className="space-y-1">
            {category.steps.map((step, i) => (
              <div
                ...
                className="p-2 border rounded text-sm cursor-grab hover:bg-accent/50 transition-colors"
              >
                {step.name}
              </div>
            ))}
...
function TriggerEditor({ triggers, onChange }: {
  ...
  const toggleTrigger = (trigger: string) => {
    if (activeTriggers.includes(trigger)) {
      ...
      onChange({
        ...triggers,
        [trigger]: getDefaultTriggerConfig(trigger),
      });
    }
  };
  ...
  return (
    <div className="space-y-3">
      {['push', 'pull_request', 'workflow_dispatch', 'schedule'].map((trigger) => (
        <div key={trigger} className="space-y-2">
          ...
          {activeTriggers.includes(trigger) && (
            <TriggerConfig
              type={trigger}
              config={triggers[trigger as keyof WorkflowTrigger]}
              onChange={(config) => onChange({ ...triggers, [trigger]: config })}
            />
          )}
        </div>
      ))}
...
function getDefaultTriggerConfig(trigger: string) {
  switch (trigger) {
    case 'push': return { branches: ['main'] };
    case 'pull_request': return { branches: ['main'] };
    case 'workflow_dispatch': return {};
    case 'schedule': return [{ cron: '0 0 * * *' }];
    default: return {};
  }
}
```
...
- `apps/web/src/routes/repo/workflow-editor.tsx` - New file (main editor)
- `apps/web/src/App.tsx` - Add route
- `apps/web/src/routes/repo/workflows.tsx` - Add "New Workflow" button linking to editor
...
```tsx
// In App.tsx
<Route path="actions/new" element={<WorkflowEditor />} />
<Route path="actions/edit" element={<WorkflowEditor />} />
```

## Testing
1. Navigate to Actions > New Workflow
2. Add/remove triggers
3. Add jobs with steps
4. Drag steps to reorder
5. Switch to YAML view, verify output
6. Edit YAML, switch back to visual
7. Save workflow, verify file created
8. Edit existing workflow

## Success Criteria
- [ ] Visual drag-and-drop step ordering
- [ ] Real-time YAML generation
- [ ] YAML parsing with error display
- [ ] Trigger configuration UI
- [ ] Job dependency selection
- [ ] Step library with templates
- [ ] Monaco editor for YAML view
- [ ] Save creates/updates workflow file
