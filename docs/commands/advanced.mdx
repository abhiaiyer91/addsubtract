...
Break down large features into smaller, reviewable branches that build on each other.
...
<Info>
For a complete guide, see [Stacked Diffs](/commands/stacked-diffs).
</Info>

### Quick Start

```bash
# Create a stack from main
wit stack create auth-feature

# Create branches as you work
wit stack push                    # Creates auth-feature/part-1
wit add . && wit commit -m "Add login"

wit stack push                    # Creates auth-feature/part-2  
wit add . && wit commit -m "Add logout"

# View the stack
wit stack show

# Sync after main updates
wit stack sync
