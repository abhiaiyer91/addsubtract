...
wit includes built-in AI capabilities powered by [Mastra](https://mastra.ai/), supporting OpenAI and Anthropic models.
...
- Understand complex commits and diffs
...
wit's AI agent has access to specialized tools:

| Tool | Purpose |
|------|---------|
| `getStatus` | Understand repository state |
| `getDiff` | Analyze changes |
| `getLog` | Review commit history |
| `getBranches` | Navigate branches |
| `stageFiles` | Stage files for commit |
| `createCommit` | Create commits |
| `switchBranch` | Switch branches |
| `getMergeConflicts` | View merge conflicts |
| `resolveConflict` | Resolve conflicts |
| `undo` | Undo operations |
| `search` | Search commits, files, content |
