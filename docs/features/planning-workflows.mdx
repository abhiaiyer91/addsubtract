...
description: "Plan complex tasks with AI and execute them with parallel coding agents"
---

# Planning Workflows

Planning Workflows is a powerful feature that helps you break down complex development tasks into discrete, parallelizable units of work, and then execute them with multiple AI coding agents running simultaneously.
...
const tasks = await finalizeTasks({
  sessionId: session.id,
  tasks: [
    { title: 'Create auth types', description: '...', priority: 'high' },
    { title: 'Implement password hashing', description: '...', priority: 'high' },
    // ...
  ],
});
