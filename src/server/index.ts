...
import { registerNotificationHandlers, registerCIHandlers, registerTriageHandlers, registerMergeQueueHandlers } from '../events';
...
export function createApp(repoManager: RepoManager, options: { verbose?: boolean } = {}): Hono {
  ...
  const DOCS_HOST = 'test-4cc74a53.mintlify.dev';
...
export function startServer(options: ServerOptions): WitServer {
  ...
  if (databaseUrl) {
    try {
      ...
      // Register event handlers for notifications, CI, triage, and merge queue
