...
tsgit handles large files natively with automatic chunking - no Git LFS required.

## Overview

Unlike Git, which struggles with large files and requires Git LFS for efficient handling, tsgit includes built-in large file support:

- **Automatic chunking:** Large files are split into manageable chunks
- **Deduplication:** Identical chunks are stored only once
- **Streaming:** Large files don't need to fit in memory
...
Set the large file threshold in `.tsgit/config`:
...
<AccordionGroup>
  <Accordion title="File not being chunked">
    Check the threshold:
    ```bash
    tsgit config tsgit.largeFileThreshold
    ```
